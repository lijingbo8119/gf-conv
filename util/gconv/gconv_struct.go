...
import (
	"errors"
	"fmt"
	"github.com/lijingbo8119/gf-conv/internal/empty"
	"github.com/lijingbo8119/gf-conv/internal/json"
	"github.com/lijingbo8119/gf-conv/internal/structs"
	"reflect"
	"strings"

	"github.com/lijingbo8119/gf-conv/internal/utils"
)
...
func doStruct(params interface{}, pointer interface{}, mapping map[string]string, priorityTag string) (err error) {
	...
	if v, ok := pointer.(reflect.Value); ok {
		...
		if pointerReflectKind != reflect.Ptr {
			return fmt.Errorf("object pointer should be type of '*struct', but got '%v'", pointerReflectKind)
		}
	...
	if paramsMap == nil {
		return fmt.Errorf("convert params to map failed: %v", params)
	}
...
func bindVarToStructAttr(elem reflect.Value, name string, value interface{}, mapping map[string]string, priorityTag string) (err error) {
	...
	defer func() {
		if e := recover(); e != nil {
			err = bindVarToReflectValue(structFieldValue, value, mapping, priorityTag)
		}
	}()
	// Directly converting.
